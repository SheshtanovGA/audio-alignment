...
def test_session_artifact_paths():
    paths = io_utils.session_artifact_paths("session3", artifacts_dir="artifacts")
    assert paths["ts_ref"] == os.path.join("artifacts", "session3", "ts_ref.npy")
    assert paths["path_stream"] == os.path.join("artifacts", "session3", "path_stream.npy")


def test_resolve_alignment_paths_from_session():
    paths = io_utils.resolve_alignment_paths(session="session2")
    assert paths["ts_stream"] == os.path.join("artifacts", "session2", "ts_stream.npy")
...
def test_resolve_alignment_paths_override():
    ...
    assert paths["ts_ref"] == os.path.join("artifacts", "session2", "ts_ref.npy")
...
def test_resolve_alignment_paths_missing():
    with pytest.raises(ValueError, match="Missing alignment artifacts"):
